...
async function on_load() {
    ...
    if (key) {
        ...
        document.getElementById("matches-from-header").textContent =
            "Matches from";
        updateMatches(form);
    }
}
        ...
        matches.appendChild(
            bdoc.ele(
                ...
                bdoc.ele(
                    "span",
                    bdoc.class("con_matchKey"),
                    "Palet Key: " + StripKeyPrefix(desc.key)
                )
            )
        );
    }
...
function minimizeDisplay() {
    ...
    if (arrow.classList.contains("fa-angle-up")) {
        ...
        buttonSpan.style =
            "display: none; justify-content: center; align-items: center; padding: 0.5em;";
        arrow.outerHTML = `<i id ="shrinker" class="fa-solid fa-angle-down" style=" cursor: pointer;float:right;"></i>`;
    } else {
        algForm.style = "";
        buttonSpan.style =
            "display: flex; justify-content: center; align-items: center; padding: 0.5em;";
