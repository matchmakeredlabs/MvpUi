...
function updateVisualStatus() {
  ...
  MmCollection.checkStatus = checkParents(
    MmCollection.depthParentSiblingMap,
    MmCollection.checkStatus
  );
  ...
  checkboxes.forEach((checkbox) => {
    ...
    inputElement.previousElementSibling.checked =
      MmCollection.checkStatus.get(Number(nodeId)) || false;
...
function createDepthParentSiblingMapFromParentMap(parentMap) {
  ...
  const seen = new Set(); // Keep track of visited nodes
  const queue = [{ node: 0, depth: 0 }]; // Start with the root node at depth 0
  seen.add(0); // Mark the root node as visited
...
function getAncestors(leafNodes, parentMap) {
  ...
  function findAncestors(node) {
    if (parentMap[node] !== undefined) {
      // If there's a parent for the node
      const parent = parentMap[node];
      if (parent !== null && !ancestors.has(parent)) {
        // Avoid adding null and duplicates
...
function previewCustomSet() {
  ...
  let allAncestors = getAncestors(
    customSet["leafNodes"],
    MmCollection.parentMap
  );

  let finalDescriptorsArr = [
    ...new Set([...allAncestors, ...customSet["leafNodes"]]),
  ];
...
export default class MmCollection {
  ...
  attachTo(element) {
    ...
    document.getElementById("match-collections-button").onclick =
      previewCustomSet;
    ...
    if (
      currentCustomSet &&
      currentCustomSet.associatedCollectionId ==
        MmCollection.thisCollection.descriptors[0].id
    ) {
