...
export default class MmSelectSubset extends HTMLElement {
    ...
    previewCustomSet = () => {
        ...
        // Get all descriptors with checked === true
        const checkedDescriptors = Object.values(descriptors).filter(
        ...
        const finalDescriptorsArr = Array.from(new Set([...customSet.leafNodes, ...allAncestors]));
        ...
        finalDescriptorsArr.forEach(intId => {
          ...
          currentDescriptor.parent = parentMap[intId] !== undefined ? parentMap[intId] : null;
        ...
        // Save the custom set object to localStorage.
        ...
        // Redirect to the PreviewCustomSet page.
        ...
        Promise.all([
            ...
            const generateCustomDescriptorElement = (element) => {
                ...
                checkbox.addEventListener("change", (e) => {
                    // Helper functions
                    const updateDescendants = (descriptor, isChecked, descriptorsMap) => {
                        descriptor.checked = isChecked;
                        
                        if (descriptor.intHasPart && descriptor.intHasPart.length) {
                            descriptor.intHasPart.forEach((childId) => {
                                const child = descriptorsMap[childId];
                                if (child) {
                                    updateDescendants(child, isChecked, descriptorsMap);
                                }
                            });
                        }
                    };

                    const updateAncestors = (descriptor, descriptorsMap) => {
                        ...
                        if (descriptor.isPartOfId) {
                            ...
                            if (parent) {
                                ...
                                updateAncestors(parent, descriptorsMap);
                            }
                        }
                    };
                    ...
                    updateDescendants(element, isChecked, collectionEle.descriptors);
                    
                    // When unchecking, uncheck all of its ancestors
                    if (!isChecked) {
                        updateAncestors(element, collectionEle.descriptors);
                    }
                    ...
                    if (li) {
                        ...
                        while (parentLI) {
                            const ancestorCheckbox = parentLI.querySelector("input[type='checkbox']");
                            if (ancestorCheckbox && !isChecked) {
                ...
                return bdoc.ele("div", 
                    ...
                    bdoc.attr("style", "display: inline-flex; gap: 0.5em; margin-right:0.5em;"), checkbox);
            };
