...
class Mmx {
  ...
  static descProps = [
    {
      ...
      prop: "eleType",
      inner:
        "<select name='eleType'><option value='o'>Other</option><option value='lr'>Learning Resource</option><option value='cs'>Competency Statement</option><option value='c'>Curriculum</option></select>",
    },
    ...
    parent.appendChild(
      bdoc.ele(
        ...
        bdoc.ele(
          ...
          bdoc.ele("span", bdoc.attr("innerHTML", "Search Type:&nbsp;&nbsp;")),
          bdoc.ele(
            "div",
        ...
        bdoc.ele(
          "div",
          bdoc.ele("span", bdoc.attr("innerHTML", "AI Algorithm:&nbsp;&nbsp;")),
          bdoc.ele(
            "select",
  ...
  static RenderLrmiForm(element) {
...

    {
      ...
      controlsLeft.appendChild(
        bdoc.ele("button", bdoc.attr("onclick", Mmx.MatchDescriptor), "Match")
      );
      controlsLeft.appendChild(document.createTextNode(" "));
      controlsLeft.appendChild(
        bdoc.ele("button", bdoc.attr("onclick", Mmx.ClearLrmiForm), "Clear")
      );
      controlsLeft.appendChild(document.createTextNode(" "));
      controlsLeft.appendChild(
        bdoc.ele("button", bdoc.attr("onclick", Mmx.SaveDescriptor), "Save")
      );
      ...
      controlsRight.appendChild(
        bdoc.ele("button", bdoc.attr("onclick", Mmx.PrevDescriptor), "←")
      );
      controlsRight.appendChild(document.createTextNode("\u00A0")); // Insert a non-breaking space
      controlsRight.appendChild(
        bdoc.ele("button", bdoc.attr("onclick", Mmx.NextDescriptor), "→")
      );
  ...
  static OnPageLoad(keyParent) {
    ...
    const downloadJsonOrCsv = (fileType) => {
        let matchThresholdInput = document.getElementById("match-threshold");

        let url = `/descriptors?searchKey=${mmx_dict.searchKey}&eleType=${
          mmx_dict.searchEleType
        }&${jsonToQueryString(localStorage.getItem("matchWeightsObj"))}&matchThreshold=${matchThresholdInput.value}`;
