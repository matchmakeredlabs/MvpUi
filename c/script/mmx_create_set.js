...
function handleCheckboxClick(clickedCheckbox) {
  ...
  if (clickedCheckbox.checked) {
    sessionStorage.setItem("selectedCheckbox", clickedCheckbox.id);
  } else {
    sessionStorage.removeItem("selectedCheckbox");
  }
...
async function initialLoad(listToFilter, listToSort, displayProperties) {
  ...
  for (const collection of data.collections) {
    ...
    elementData += `<td style = "text-align: center;"><input type="checkbox" id=${collection["id"]}></td>`;
    // console.log("pikachu", elementData);
  ...
  function createSetOnclick() {
    if (!isAnyCheckboxChecked()) {
      alert("Error: You must select a collection to create a custom set.");
    } else {
  ...
  if (searchKeywords.length > 0) {
    for (const collection of data.collections) {
      ...
      for (let keyword of searchKeywords) {
        if (
          stringifiedCollection.toLowerCase().includes(keyword.toLowerCase())
        ) {
          keywordCount += 1;
        }
      }
